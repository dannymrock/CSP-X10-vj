...
public class MagicSquareAS(squareLength:Long) extends ModelAS{   
	 ...
	 */
	public def this(length:Long, vectorSize:Long/*{self==length*length}*/, seed:Long){
		super(vectorSize, seed);
		property(length);
		assert length*length==vectorSize;
		initParameters();
	}
	...
	private def initParameters(){
		...
		avg = (squareLength * (length + 1n) / 2n) as Int;		/* sum to reach for each l/c/d */
		...
		square_length_m1 = (squareLength - 1) as Int;
		square_length_p1 = (squareLength + 1) as Int;
		...
		{	
			...
			val i = (k as Int / squareLength) as Int;
			val j = (k as Int % squareLength) as Int;
